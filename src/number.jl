struct Number{T <: AbstractVector{UInt8}} <: Base.Number
    buf::T
    pos::Int
end

Base.show(io::IO, x::Number) = print(io, "JSONBase.Number(", Base.Number(x), ")")
...
Base.promote_rule(::Type{Number}, ::Type{T}) where {T <: Base.Number} = T
Base.convert(::Type{T}, x::Number) where {T <: Base.Number} = T(Base.Number(x))
Base.getindex(x::Number) = Base.Number(x)

materialize(x::Number) = parsenumber(Base.Number, x)

@enum NumberError Overflow ExpectedDigit

(::Type{T})(n::Number) where {T <: Base.Number} = parsenumber(T, n)[1]

function parsenumber(::Type{T}, n::Number) where {T}
    buf, pos = getbuf(n), getpos(n)
    len = length(buf)
    spos = pos
    @nextbyte(false)
    # begin parsing
    ...
    @nextbyte(false)
    if b != UInt('.') && (b - UInt8('0') > 0x09)
        #TODO: check for nan/inf
        error = ExpectedDigit
        @goto invalid
    end
    # parse rest of number
    digits = T <: Integer ? T(0) : Int64(0)
    x, pos = parsedigits(T, buf, pos, len, b, digits, neg, spos)
...
function _widen(v::UInt128)
    x = access_threaded(() -> BigInt(; nbits=256), BIGINT)
...
_muladd(ten, digits::T, b) where {T} = ten * digits + T(b)

function _muladd_over(ten, digits::T, b) where {T}
    digits, ov_mul = Base.mul_with_overflow(digits, ten)
    digits, ov_add = Base.add_with_overflow(digits, T(b))
    return digits, (ov_mul | ov_add)
end
...
_muladd_over(ten, digits::BigInt, b) = (_muladd(ten, digits, b), false)
...
@noinline _parsedigits(::Type{T}, buf, pos, len, b, digits, neg::Bool, spos) where {T} =
    parsedigits(T, buf, pos, len, b, digits, neg, spos)

@inline function parsedigits(::Type{T}, buf, pos, len, b, digits::IntType, neg::Bool, spos) where {T, IntType}
    ndigits = 0
    overflowmode = false
    ...
    if b != UInt8('.')
        ...
        while true
            if overflowmode
                newdigits, over = _muladd_over(ten(IntType), digits, b)
                if over
                    if T >: Real
                        return _parsedigits(T, buf, pos, len, b + UInt8('0'), _widen(digits), neg, spos)
                    else
                        error = Overflow
                        @goto invalid
                    end
    ...
    if b == UInt8('.')
        ...
        if pos > len
            ...
            x = ifelse(neg, -T(digits), T(digits))
            if T >: Real
                x = Float64(x) # will this ever be wrong for really large integers?
            end
            @goto done
        end
        b = getbyte(buf, pos)
    ...
    x, pos = parsefrac(T, buf, pos, len, b, digits, neg, spos, UInt64(0))
...
@noinline _parsefrac(::Type{T}, buf, pos, len, b, digits::IntType, neg::Bool, spos, frac) where {T, IntType} =
    parsefrac(T, buf, pos, len, b, digits, neg, spos, frac)

@inline function parsefrac(::Type{T}, buf, pos, len, b, digits::IntType, neg::Bool, spos, frac) where {T, IntType}
    parsedanyfrac = false
    ...
    if b - UInt8('0') < 0x0a
        ...
        while true
            ...
            b > 0x09 && break
            if overflows(IntType) && digits > overflowval(IntType)
                return _parsefrac(T, buf, pos, len, b + UInt8('0'), _widen(digits), neg, spos, frac)
            end
        end
        b += UInt('0')
    end
    ...
    if b == UInt8('e') || b == UInt8('E') || b == UInt8('f') || b == UInt8('F')
        ...
        return parseexp(T, buf, pos, len, b, digits, neg, spos, frac, UInt64(0), negexp)
    else
...
@noinline _parseexp(::Type{T}, buf, pos, len, b, digits, neg::Bool, spos, frac, exp::ExpType, negexp) where {T, ExpType} =
    parseexp(T, buf, pos, len, b, digits, neg, spos, frac, exp, negexp)

@inline function parseexp(::Type{T}, buf, pos, len, b, digits, neg::Bool, spos, frac, exp::ExpType, negexp) where {T, ExpType}
    ...
    while true
        ...
        if overflows(ExpType) && exp > overflowval(ExpType)
            return _parseexp(T, buf, pos, len, b, digits, neg, spos, frac, _widen(exp), negexp)
        end
    end
end
...
function scale(::Type{T}, v, exp, neg) where {T}
    return __scale(T <: AbstractFloat ? T : Float64, unsigned(v), exp, neg)
end
